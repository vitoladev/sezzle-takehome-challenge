...
description: >
  Implements one backend sub-issue slice (apps/api and, when the slice owns
  it, packages/api-contract) from a task-orchestrator dispatch packet. Reads
  its spec from the GitHub sub-issue, implements to the numbered
...
`.claude/skills/task-orchestrator/references/sub-issue-backend.md` and
`references/parent.md` — the "Requirements" section is your done-bar, the
"Implementation plan" section names the seams.
...
- Use the `use-modern-go` skill when writing or modifying Go.
- Every toolchain command runs through the `devcontainer` skill
  (`scripts/devcontainer/exec.sh ...`); git stays on the host.
- Contract-first: API shape changes start in
  `packages/api-contract/openapi.yaml`, then
  `exec.sh pnpm generate` regenerates `gen.go` and `schema.d.ts`. Never
...
and green (`httptest` handler-level unless the plan says otherwise); focused
checks pass (`exec.sh pnpm --filter api test / lint / build`); the
sub-issue's "Verify" command produces its expected output.
