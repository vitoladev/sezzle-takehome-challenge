...
test.describe('Parent acceptance criteria', () => {
...
test.describe('The app never evaluates arithmetic (requirement 3)', () => {
...
test.describe('One Calculation per press, and the guard always releases (WEB-8)', () => {
  ...
  test('clearing while a Calculation is in flight does not deadlock the = key', async ({
    page,
  }) => {
    const w = watch(page)
...
test.describe('Every error form renders distinguishably (req 7)', () => {
  const ERROR_FORMS = [
  ...
  for (const form of ERROR_FORMS) {
    test(`${form.code} renders its own code, the API's message and a retry`, async ({
      ...
      if (!('error' in expected)) throw new Error('the API no longer refuses this Calculation')
      expect(expected.error, 'the API still answers this Calculation with this form').toBe(
        form.code,
      )
...
test.describe('The three intentional states', () => {
  ...
  test('pending — a delayed API leaves a real pending state with = disabled', async ({
    page,
  }) => {
    const w = watch(page)
...
test.describe('The Session identifier (requirements 4 and 5)', () => {
...
test.describe('Keypad and keyboard (requirement 1)', () => {
...
test.describe('Presentation (requirements 10 and 11)', () => {
